...
export default function MobileMenu() {
  ...
  return status !== "loading" && <div
    ...
    {[...menuConfig].sort((a, b) => a.mobileIndex - b.mobileIndex).map(item => {
      const MenuIcon = item.icon;
      return (!item.requiresAuth || (item.requiresAuth && session) )&& <MenuLink key={item.route} href={item.route}>
        <MenuIcon/>
      </MenuLink>
    })}
  </div>
